...
WORKDIR /genpaylib
ENV GENPAY_LIB="/genpaylib"

COPY --from=builder /compiler/target/release/genpay /usr/local/bin
COPY --from=builder /compiler/stdlib /genpaylib
...
CMD ["genpay", "source.genpay", "output", "&&", "./output"]
